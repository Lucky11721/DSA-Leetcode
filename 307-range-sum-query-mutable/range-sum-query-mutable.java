class NumArray {
    ...
    public void update(int index, int val) { 
        updateSGTree(index , val , 0 , 0 , n-1);

         }
         public void updateSGTree(int index , int val , int idx , int i , int j){
            if(i == j){
                segmentTree[idx] = val;
                return;
            }
            ...
            if(mid >= index){
                updateSGTree(index , val , 2*idx+1 , i , mid);
            }
            else{
                updateSGTree(index , val , 2*idx+2 , mid+1 , j);
            }
